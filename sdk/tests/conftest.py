...
async def _reset_global_singletons() -> AsyncGenerator[None, None]:
    ...
    thread (Database) into whatever test runs next. Since pytest-asyncio gives
    each test function its own loop, that is an ordering-dependent failure
    waiting to happen — reset both unconditionally after every test rather
