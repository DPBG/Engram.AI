...
class FakeNetwork:
    ...
    def step(self, sensory_current=None):
        ...
        self.concept.set_active(
            [(self._step_call_count * 7 + i) % self.concept.n for i in range(self.concept._k)]
        )
        self.regions["sensory"]._spikes[:] = False
        self.regions["sensory"]._spikes[:50] = True
        return {"motor_commands": [], "prediction_error": 0.1, "step_count": self._step_count}
...
class TestConceptProbe:
    ...
    async def test_basic_probe(self):
        ...
        result = await run_concept_probe(
            net,
            {
    ...
    async def test_different_stimuli_different_patterns(self):
        ...
        net = FakeNetwork()
        result_dog = await run_concept_probe(
            net,
            {
                "stimulus": {"provenance": "observation.text", "data": "dog"},
                "propagation_steps": 3,
        ...
        result_cat = await run_concept_probe(
            net,
            {
    ...
    async def test_accumulation_across_steps(self):
        ...
        net1 = FakeNetwork()
        result_short = await run_concept_probe(
            net1,
            {
                "stimulus": {"provenance": "observation.text", "data": "test"},
                "propagation_steps": 1,
        ...
        net2 = FakeNetwork()
        result_long = await run_concept_probe(
            net2,
            {
                "stimulus": {"provenance": "observation.text", "data": "test"},
